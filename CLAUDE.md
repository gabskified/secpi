...
4. **Every flag change is a diff to `docs/FLAGS.md`.** Flags carry stable numbers (#1–#41 assigned; next free number is **#42**). Never renumber.
...
These must be regenerated by running the pipeline under Option B, **after** the two open normalization/statistics decisions in `docs/DECISIONS.md` are settled. The code is ready to run as-is for Option B; no code change is required for that decision itself.
...
│   ├── FLAGS.md               ← the 41-flag editorial register (v2), next number #42
│   └── proposals/
