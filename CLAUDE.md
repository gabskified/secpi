...
4. **Every flag change is a diff to `docs/FLAGS.md`.** Flags carry stable numbers (**#1–#95 assigned; next free number is #96** — updated 2026-07-26, Project Log Entry 6). Never renumber. Do not trust a flag total quoted anywhere else; **`docs/FLAGS.md` is the register, and `docs/STATE.md`'s authoritative count block is the count.**
...
| Land-use classes | P (public) 55–65%, A (accessible) 25–40%, V (vulnerable) 5–10% |
...
These must be regenerated by running the pipeline under Option B. The code is ready to run as-is for Option B; no code change is required for that decision itself.

> ✅ **Updated 2026-07-26 (Project Log Entry 7 + addendum).** This paragraph previously said regeneration waited on *"the two open normalization/statistics decisions."* **Both are now closed, along with two others. Every decision on the Results-regeneration critical path is settled** — D-01, D-02 (ceiling **3.75**), D-03 (report **both** metrics H1/H2, paired Wilcoxon n = 30, **Holm–Bonferroni** m = 2 FWER 0.05), D-07 (**`s`** = species subset/palette size, **`k`** = tree count), D-11 (**option (b)** — full §3.5 regeneration under Option B).
>
> **Regeneration is no longer decision-blocked. It is blocked on two flags settling first:**
> - **#75** — Methods §2.5.3 names the **Morris method** while Results §3.5.1 executes a **local two-level OAT** from a single baseline. Regenerating before this resolves reproduces the mismatch in fresh numbers.
> - **#77** — the sensitivity sweep averaged over **three** ACO runs against a project standard of `n_runs = 5`.
>
> **Two sequencing constraints that are easy to violate:** D-03 condition 2 requires `editor` to write H1 and H2 into Methods §2.5.2 **before** `code-stressor` executes the tests — running first voids the pre-specification. And D-02's 3.75 ceiling carries a **standing re-check**: it sits just above 3.52, the max from 500 *random* placements, and the ACO optimizes harder than random, so the optimizer's best raw SECPI must be compared against it before any Results prose is written.
>
> **Also now on record: the project's first ROADBLOCK (SEVERE), Flag #82** — §3.5.2's four category-level mean sensitivity indices each exceed the maximum SI of their own member set. It blocks the preprint independently of everything above. D-11 sets its remedy; the remedy has not been applied. Four decisions remain open: **D-04** (final title — the only DOI-blocking item), **D-05**, **D-08**, **D-10**.
...
│   ├── FLAGS.md               ← the 95-flag editorial register (v4), next number #96
│   └── proposals/
...
| **Results §3.1–§3.4.4** | Reviewed | ~~NOT YET REVIEWED~~ — first editorial pass 2026-07-25 (Entry 5), flags **#52–#74**. 13 of 23 are POTENTIAL ROADBLOCK. Still blocked on regeneration (§4); §3.4.1 must be **regenerated, not rewritten** (#64). |
| **Results §3.5 (Sensitivity)** | Reviewed | ~~NOT YET REVIEWED~~ — pass 2026-07-26 (Entry 6), flags **#75–#81, #83–#86**. Carries **#82, the project's first ROADBLOCK (SEVERE)**. Regeneration scope decided (D-11 option b) but **gated on #75 and #77**. |
| **Discussion** | Reviewed | Not a separate file — Discussion is interleaved with Results in `06_results_discussion.md` and was covered by the two passes above. |
| **Conclusion / Recommendations / back matter** | Reviewed | ~~NOT YET REVIEWED~~ — pass 2026-07-26 (Entry 6), flags **#87–#95**. Inherits the °C, 42% and 3.13 defects; #88 reports a robustness result absent from §3. |

> **Every manuscript section now has at least one editorial pass** (updated 2026-07-26, Entry 6). Entry 4's handoff note 3 is fully discharged. "Reviewed" here means *editorially flagged*, **not** *corrected* — no flag above has been applied to the prose, and Results cannot be rewritten until regeneration completes.
